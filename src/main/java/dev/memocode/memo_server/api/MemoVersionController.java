...
public class MemoVersionController implements MemoVersionApi {
    ...
    public ResponseEntity<Void> deleteMemoVersion(@PathVariable UUID memoId, @PathVariable UUID memoVersionId, @AuthenticationPrincipal Jwt jwt){

        return ResponseEntity.ok().build();
    }

    @GetMapping("/{memoVersionId}")
    public ResponseEntity<MemoVersionDetailDTO> findMemoVersion(@PathVariable UUID memoId, @PathVariable UUID memoVersionId, @AuthenticationPrincipal Jwt jwt){

        return ResponseEntity.ok().build();
    }

    @GetMapping
    public ResponseEntity<MemoVersionsDTO> findAllMemoVersion(@PathVariable UUID memoId, @AuthenticationPrincipal Jwt jwt){
        return ResponseEntity.ok().build();
    }
